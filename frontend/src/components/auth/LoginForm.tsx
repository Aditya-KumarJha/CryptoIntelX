...
export default function LoginForm({ setOtpStep, setUserEmail, setForgotStep, errorMessage }: Props) {
  ...
  const handleSocialLogin = (provider: string) => {
    ...
    switch (provider) {
      ...
      case "GitHub":
        ...
        break;
      case "Discord":
        window.location.href = `${baseUrl}/api/auth/discord?mode=login`;
        break;
      case "LinkedIn":
  ...
  return (
    <div className="bg-white/80 dark:bg-black/80 backdrop-blur-md p-10 flex flex-col justify-center text-gray-900 dark:text-white rounded-r-2xl">
      <h2 className="text-2xl font-bold text-center mb-2">Login to CryptoIntelX</h2>
      <p className="text-sm text-gray-600 dark:text-gray-300 text-center mb-8">
        Access your account to track suspicious wallets and monitor blockchain activity securely.
      </p>
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <span className="text-xs text-gray-500 dark:text-gray-400">or login with email</span>
        <div className="h-px bg-gray-300 dark:bg-gray-700 flex-1" />
      </div>
